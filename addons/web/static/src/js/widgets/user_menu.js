...
var UserMenu = Widget.extend({
    ...
    on_menu_documentation: function () {
        window.open('https://www.elitaco.com/page/ees', '_blank');
    },
        ...
        this.trigger_up('clear_uncommitted_changes', {
            callback: function() {
                ...
                P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                    ...
                    framework.redirect('https://accounts.elitaco.com/oauth2/auth?'+$.param(params));
                }).fail(function(result, ev){
                    ev.preventDefault();
                    framework.redirect('https://accounts.elitaco.com/account');
                });
            },
        });
    },
