        ...
        "click .o_mail_request_permission": function (event) {
            ...
            if (def) {
                def.then(function (value) {
                    if (value === 'granted') {
                        utils.send_notification(_t('Permission granted'), _t('EES has now the permission to send you native notifications on this device.'));
                    } else {
                        utils.send_notification(_t('Permission denied'), _t('EES will not have the permission to send native notifications on this device.'));
                    }
                });
            }
        },
